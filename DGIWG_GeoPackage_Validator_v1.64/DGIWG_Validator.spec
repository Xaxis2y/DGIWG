...
#
# PyInstaller build spec for the DGIWG GeoPackage Compliance Validator v1.64.
#
...
#     dist\DGIWG_Validator_v1_64.exe
#
...
APP_NAME   = "DGIWG_Validator_v1_64"
ENTRY_FILE = os.path.join(HERE, "DGIWG_Validator_v1_64.py")
CACHE_FILE = os.path.join(HERE, "dgiwg_epsg_cache.json")

# Conda keeps several runtime DLLs in Library/bin rather than beside the
# extension modules that depend on them. PyInstaller cannot always resolve
# those DLLs automatically, so bundle the exact runtime dependencies required
# by sqlite3, ctypes, and tkinter.
CONDA_BIN = os.path.join(sys.prefix, "Library", "bin")
RUNTIME_DLL_NAMES = ("sqlite3.dll", "ffi.dll", "tcl86t.dll", "tk86t.dll")
binaries = []
for _dll_name in RUNTIME_DLL_NAMES:
    _dll_path = os.path.join(CONDA_BIN, _dll_name)
    if os.path.isfile(_dll_path):
        binaries.append((_dll_path, "."))
    else:
        print(f"  [spec] WARNING: required Conda runtime DLL not found: {_dll_path}")
