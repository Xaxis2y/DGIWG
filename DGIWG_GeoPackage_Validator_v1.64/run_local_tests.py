...
"""
DGIWG Validator v1.64 — Local Self-Test Harness (Anaconda Prompt)
=================================================================
...
runtime failures on these machines:
    ...
    cd C:\\Users\\Son\\Documents\\DGIWG\\DGIWG_GeoPackage_Validator_v1.64
...
The script:
  ...
  5. Regression tests carried over from v1.59:
       • version stamp is consistent in package, VERSION.txt, launcher, and JSON schema
       ...
       • current QUICKSTART, manual, packager, cache, and launcher exist
...
# ──────────────────────────────────────────────────────────────────────────────
def base_schema(conn, valid_version=True):
    c = conn.cursor()
    c.execute("PRAGMA application_id = %d" % (0x47504B47 if valid_version else 0x47503132))
    c.execute("PRAGMA user_version = %d" % (10400 if valid_version else 10300))
    c.executescript("""
...
def make_good_tiles(path):
    ...
    c = base_schema(conn, valid_version=True)
...
def make_bad_mixed(path):
    ...
    if os.path.exists(path):
        os.remove(path)
    conn = sqlite3.connect(path)
    c = base_schema(conn, valid_version=False)   # violations: legacy GP12 and old user_version
    c.execute("INSERT INTO gpkg_spatial_ref_sys VALUES "
              "('OSGB36 / British National Grid', 27700, 'EPSG', 27700, ?, NULL)",
              (WKT1_27700,))                     # violation: CRS 27700 is not allowed
    c.executescript("""
    CREATE TABLE gpkg_tile_matrix_set (
      table_name TEXT PRIMARY KEY, srs_id INTEGER NOT NULL,
      min_x DOUBLE NOT NULL, min_y DOUBLE NOT NULL,
...
def main():
    ...
    if proc.returncode != 2:
        failures.append(f"batch run exit code {proc.returncode} (expected 2 because junk.gpkg is invalid)")
    checks += 1
    ...
    if reports.get("bad_mixed.gpkg", {}).get("verdict") != "NON-CONFORMANT (automated checks)":
        failures.append("bad_mixed.gpkg verdict is not NON-CONFORMANT")
    if reports.get("features.gpkg", {}).get("verdict") != "NON-CONFORMANT (automated checks)":
    ...
    try:
        ...
        _ecc = base_schema(_ec, valid_version=True)
    ...
    # 20. v1.64 standards and error-semantics regressions ----------------------
    log("STEP: v1.64 - GeoPackage 1.4 identifiers, WKT1, ERROR, and coverage semantics")
    from dgiwg_validator.checks import (
        _CHECK_DISPATCH, _compound_wkt_components, _r3, _r9, check_req,
    )
    from dgiwg_validator.constants import PASSSTAR_EVIDENCE_MISSING
    from dgiwg_validator.utils import score_results

    _gp12_path = os.path.join(TEST_ROOT, "legacy_gp12.gpkg")
    shutil.copy2(os.path.join(GPKG_DIR, "good_tiles.gpkg"), _gp12_path)
    with sqlite3.connect(_gp12_path) as _db:
        _db.execute("PRAGMA application_id = 1196437809")  # 0x47503131 is replaced below
        _db.execute("PRAGMA application_id = 1196437810")  # 0x47503132, legacy GP12
        _db.execute("PRAGMA user_version = 10400")
        _status, _detail = _r3(_db.cursor())
    checks += 1
    if _status != "FAIL" or "expected 0x47504B47" not in _detail:
        failures.append(f"legacy GP12 application_id was not rejected: {_status} {_detail[:160]}")

    _oldver_path = os.path.join(TEST_ROOT, "old_user_version.gpkg")
    shutil.copy2(os.path.join(GPKG_DIR, "good_tiles.gpkg"), _oldver_path)
    with sqlite3.connect(_oldver_path) as _db:
        _db.execute("PRAGMA application_id = 1196444487")  # 0x47504B47, GPKG
        _db.execute("PRAGMA user_version = 10300")
        _status, _detail = _r3(_db.cursor())
    checks += 1
    if _status != "FAIL" or "not GeoPackage 1.4.x" not in _detail:
        failures.append(f"old user_version was not rejected: {_status} {_detail[:160]}")

    _wkt1_compound = (
        'COMPD_CS["WGS 84 + test height",'
        'GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",6378137,298.257223563]],'
        'PRIMEM["Greenwich",0],UNIT["degree",0.0174532925199433],'
        'AXIS["Latitude",NORTH],AXIS["Longitude",EAST]],'
        'VERT_CS["Test height",VERT_DATUM["Test datum",2005],UNIT["metre",1],'
        'AXIS["Gravity-related height",UP]]]'
    )
    _syntax, _horiz, _vert, _datum, _issues = _compound_wkt_components(_wkt1_compound)
    checks += 1
    if (_syntax, _horiz, _vert, _datum, _issues) != ("WKT1", "GEOGCS", "VERT_CS", True, []):
        failures.append(f"valid WKT1 compound CRS was not accepted: {_issues}")

    _original = _CHECK_DISPATCH[3]
    try:
        def _raise_for_test(_cursor):
            raise RuntimeError("forced test exception")
        _CHECK_DISPATCH[3] = _raise_for_test
        _status, _detail, _reason = check_req(None, 3)
    finally:
        _CHECK_DISPATCH[3] = _original
    checks += 1
    if _status != "ERROR" or "forced test exception" not in _detail:
        failures.append(f"internal exception did not become ERROR: {_status} {_detail}")

    _counts, _total, _verdict, _colour = score_results({
        1: {"status": "ERROR", "detail": "forced"},
        2: {"status": "FAIL", "detail": "data failure"},
    })
    checks += 1
    if _verdict != "VALIDATION ERROR" or _counts.get("ERROR") != 1:
        failures.append(f"ERROR verdict precedence is wrong: {_verdict} {_counts}")

    for _req in (8, 18):
        _row = reports.get("good_tiles.gpkg", {}).get("requirements", {}).get(str(_req), {})
        checks += 1
        if _row.get("status") != "PASS*" or _row.get("reason") != PASSSTAR_EVIDENCE_MISSING:
            failures.append(f"Req {_req} must remain PASS*/EVIDENCE_MISSING: {_row}")

    _header_path = os.path.join(TEST_ROOT, "bad_header_srs.gpkg")
    shutil.copy2(os.path.join(GPKG_DIR, "features.gpkg"), _header_path)
    with sqlite3.connect(_header_path) as _db:
        _blob = _db.execute("SELECT geom FROM parcels ORDER BY fid LIMIT 1").fetchone()[0]
        _bad_blob = _blob[:4] + struct.pack("<i", 3857) + _blob[8:]
        _db.execute("UPDATE parcels SET geom=? WHERE fid=1", (_bad_blob,))
        _db.commit()
        _status, _detail = _r9(_db.cursor())
    checks += 1
    if _status != "FAIL" or "does not match gpkg_geometry_columns.srs_id" not in _detail:
        failures.append(f"Req 9 missed a binary-header srs_id mismatch: {_status} {_detail[:200]}")
