...
"""
DGIWG Validator — Release Packaging Script (v1.64)
==================================================
...
Excluded : __pycache__, old-version launchers, superseded manuals, reports,
           local_test, logs, dist itself, build itself, any _superseded*/_to_delete* folder.

v1.64 changes:
  • applies file exclusions at every directory depth and excludes bundled
    standards/reference documents from the clean source release
  • produces source ZIP and standalone EXE as separate deliverables

v1.63 changes:
  • EXCLUDE_DIRS now also skips "build" — PyInstaller's intermediate build
    cache (dist\.. was already excluded, build\.. was not). A local .exe build
    left a 52 MB build\ folder sitting in the project root and the previous
    version of this script would have zipped every byte of it straight into
    the release archive. Re-run the .exe build any time; nothing in build\ is
    ever meant to ship.

v1.62 changes:
  • no functional change to the packager — RELEASE_NOTES_v1.62.md is picked up
    automatically because release notes are not in EXCLUDE_FILE_PATTERNS
  • the superseded v1.60 user manual lives under _superseded_v160/, which the
    EXCLUDE_DIR_PREFIXES rule below already keeps out of the archive

v1.60 changes:
  • no functional change to the packager — RELEASE_NOTES_v1.60.md is picked up
    automatically because release notes are not in EXCLUDE_FILE_PATTERNS
  • the superseded v1.59 launcher and user manual live under _superseded_v159/,
    which the EXCLUDE_DIR_PREFIXES rule below already keeps out of the archive
...
EXCLUDE_DIRS = {
    "__pycache__", "dist", "build", "reports", "local_test", ".git", "Docs",
    "manual_render_pages",
}
# v1.59 fix: folders used to park superseded files during an in-place update
...
EXCLUDE_FILE_PATTERNS = [
    ...
    # v1.59: superseded user manuals — only the current version ships
    r"^DGIWG_GeoPackage_Validator_User_Manual_v(?!%s\.docx$).*\.docx$"
    % VERSION.replace(".", r"\."),
    # v1.59: maintainer-only tooling and internal review notes — not shipped.
...
# v1.59: a release is not allowed to ship without these.  Missing any one of
