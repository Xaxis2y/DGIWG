...
"""Rollup aggregation + DGIWG_GPKG_FINAL_REPORT.html/.csv (v1.64)."""
import os
import sys
import html
import csv as _csv_mod
from .constants import REQUIREMENTS, CONFIDENCE_COLOR, CONFIDENCE_BG
...
def aggregate_rollup(all_results):
    ...
    """
    # Requirements where the current result needs an explicit external cross-check.
    ...
    rollup = {req: {
        "applicable": 0, "validator_disputes": 0, "errors": 0,
        ...
        # v1.61: split PASS* into blocking (coverage gap) vs advisory-only,
        # mirroring utils.coverage_gaps()/advisory_reqs() at the per-req level.
        "passstar_coverage_gap": 0,
        "passstar_advisory":     0,
    ...
    for filename, results, *_ in all_results:
        ...
        for req_num, r in results.items():
            # v1.59 fix: select requirement rows by key TYPE, not by a
            # hardcoded blocklist of "__dunder__" side-channel keys.
            ...
            if status == "PASS*":
                ...
                if _reason in PASSSTAR_BLOCKING_REASONS:
                    rollup[req_num]["passstar_coverage_gap"] += 1
                else:
                    rollup[req_num]["passstar_advisory"] += 1
                mc = r.get("manual")
...
def render_rollup_html(
    ...
    for req_num, (name, uri, compliance) in REQUIREMENTS.items():
        ...
        guidance = {
            1: "Confirm the current DGIWG profile and conformance class externally.",
            2: "Run the official DGIWG/OGC ATS and schema evidence checks.",
            6: "Confirm intended product type and profile applicability.",
            8: "Run OGC 17-083r2 TMS ATS A.1-A.16 and well-known scale-set checks.",
            18: "Validate metadata with the official DGIWG DMF schema set and ATS.",
        }.get(req_num, "Review current per-file details and Manual Confirmation panels; no historical corpus evidence is applied.")
        auditor_bullets = f"<li>{guidance}</li>"
        ...
        <tr>
          ...
          <td style="padding:8px;vertical-align:top;">
            ...
            <details style="margin-top:6px;">
              <summary style="cursor:pointer;font-size:11px;font-weight:bold;color:#555;
                              ...
                              user-select:none;">
                <span style="font-size:10px;">▶</span> 🔍 Cross-check guidance
                &nbsp;{auditor_summary}
              </summary>
          ...
          <td style="padding:8px;background:{mc_req_bg};vertical-align:top;">
            ...
            <details style="margin-top:6px;">
              ...
              <div style="margin-top:5px;font-size:12px;color:#444;">
                Review current per-file details and Manual Confirmation panels; no historical corpus evidence is applied.
              </div>
            </details>
          </td>
          <td style="padding:8px;text-align:center;background:{c_bg};">
            ...
            <div style="font-size:10px;color:#666;margin-top:4px;">Confidence is derived only from the current batch results.</div>
          </td>
        </tr>""")
...
<div style="background:#eaf2f8;border:1px solid #aed6f1;border-left:5px solid #2980b9;
            border-radius:6px;padding:12px 18px;margin:0 0 16px 0;font-size:13px;color:#21618c;">
  <strong>&#8505;&#65039; Scope of these verdicts</strong><br>
  Every verdict in this report covers the 34 requirements this tool can test automatically.
  Req 1 and Req 2 (OGC base conformance) require the OGC CITE TeamEngine test suites, and
  Req 6 (conditional extensions) depends on the applicable DGIWG product profile — all three
  are reported as SKIPPED and must be verified separately before a deliverable is declared
  fully conformant to STD-DP-19-005 v1.1.
</div>
  ...
  <tbody>
    ...
    <tr><td style="padding:8px;font-weight:bold;">Cross-Checks &amp; Diagnosis</td>
        <td style="padding:8px;">
          ...
          <strong>▶ Cross-check guidance</strong> (expandable) — current-run review guidance,
...
<div class="footer">
  ...
  Req 1–2 (OGC Base) require OGC CITE TeamEngine, and Req 6 (conditional extensions)
  requires the product profile — none of the three are automated by this script.
  <div style="margin-top:8px;font-size:11px;color:#7f8c8d !important;">SPDX-License-Identifier: GPL-2.0-or-later &nbsp;|&nbsp; Copyright (c) 2026 Eui Soo SON</div>
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(html_str)
    # v1.59 fix: honour --quiet (previously printed unconditionally).
...
def _write_rollup(
    ...
    # ── Rollup CSV ────────────────────────────────────────────────────────────
    # v1.60: `import csv as _csv_mod` was repeated here, shadowing the identical
    # module-level import.  Removed — the top-of-file import is the one used.
...
# v1.60 fix: this module ended with `if __name__ == '__main__': main()`, but
# `main` is not defined here — running `python dgiwg_validator/rollup.py`
# raised NameError (and its relative imports would have failed first anyway).
# rollup.py is a library module; the entry points are DGIWG_Validator_v1_60.py
# and `python -m dgiwg_validator`.
