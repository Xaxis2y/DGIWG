...
"""Entry-point logic for the DGIWG GeoPackage validator (v1.64).
...
def _configure_console() -> None:
    """Use UTF-8 console streams while remaining safe under frozen builds."""
...
def process_file(
    ...
    if _quiet:
        print(f"  {os.path.basename(gpkg_path):60s} {verdict:24s} "
              f"PASS={counts['PASS']} FAIL={counts['FAIL']} ERROR={counts['ERROR']} PASS*={counts['PASS*']} "
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            Use either the versioned launcher (DGIWG_Validator_v1_64.py) or
            ...
            Examples:
              ...
              python DGIWG_Validator_v1_64.py --offline myfile.gpkg
        """),
    )
    parser.add_argument(
        "inputs",
    ...
    # Stop after the first file with a conformance failure or validator error.
    parser.add_argument(
        "--fail-fast",
        action="store_true",
        default=False,
        dest="fail_fast",
        help="Stop after the first FAIL (exit 1) or ERROR (exit 2).",
    )
    ...
    if args.fail_fast:
        print("  [--fail-fast] Will stop after the first FAIL or ERROR.")
    ...
    if all_inputs:
        ...
        for i, gpkg_path in enumerate(gpkg_paths, 1):
            ...
            if result:
                ...
                if args.fail_fast:
                    ...
                    _statuses = {
                        v.get("status") for k, v in _res.items() if isinstance(k, int)
                    }
                    if "ERROR" in _statuses or "FAIL" in _statuses:
                        _code = 2 if "ERROR" in _statuses else 1
                        _kind = "validation error" if _code == 2 else "non-conformant result"
                        print(
                            f"\nFAIL-FAST: stopping after first {_kind} - {_fname}"
                        )
        ...
        has_error = had_processing_error or any(
            v.get("status") == "ERROR"
            for _name, result_set, _report in all_results
            for k, v in result_set.items() if isinstance(k, int)
        )
        has_fail = any(
            v.get("status") == "FAIL"
            for _name, result_set, _report in all_results
        ...
        sys.exit(2 if has_error else (1 if has_fail else 0))
