...
"""Utility helpers for the DGIWG validator (v1.64).
...
from .constants import PASSSTAR_BLOCKING_REASONS, PASSSTAR_DEFAULT_REASON
LIBRARY_STATUS: dict = {}
...
def _probe_optional_libraries(
    ...
    """
    Called once at startup. Checks pyproj, shapely, Pillow and lxml.
    ...
    """
    # v1.59 fix: lxml added to the probe.  _r18() uses lxml.etree.XMLSchema to
    # run the bundled DMF XSD against each metadata document, and silently
    # skips that validation when lxml is absent (`except ImportError: pass`).
    # Because lxml was not in this list, the user was never told the deepest
    # Req 18 check had been skipped, was never offered the install, and
    # LIBRARY_STATUS carried no entry for it — a metadata document with wrong
    # element order would report as clean on a machine without lxml and dirty
    # on a machine with it, with nothing in the report to explain why.
    libs = [
        ...
        ("lxml",       "lxml",        "Req 18 — DMF metadata XSD structural validation"),
    ]
...
# Envelope indicator (flags byte bits 1-3) -> envelope byte length.
# Codes 5-7 are reserved/invalid per OGC 12-128r15 Table 6.
_GPKG_ENVELOPE_SIZES = {0: 0, 1: 32, 2: 48, 3: 48, 4: 64}


def _wkb_zm_flags(wkb_data):
    """Derive (has_z, has_m, type_code) from the WKB geometry type field.

    v1.60 fix: Z and M presence is encoded in the WKB geometry *type code*,
    not in the GeoPackage header flags byte.  See ``_decode_gpkg_geom_header``
    for why the previous flags-byte reading was wrong.

    Encodings handled:

    * ISO / OGC 06-103r4 WKB — the type code carries a thousands offset:
      ``0xxx`` = XY, ``1xxx`` = XYZ, ``2xxx`` = XYM, ``3xxx`` = XYZM.
    * PostGIS EWKB — high bits on the type code: ``0x80000000`` = Z,
      ``0x40000000`` = M.  GeoPackage mandates ISO WKB, but some producers
      write EWKB, so it is decoded rather than misread.

    Parameters
    ----------
    wkb_data : bytes
        The WKB payload (byte-order byte first), i.e. the slice returned in
        ``_decode_gpkg_geom_header()["wkb_data"]``.

    Returns
    -------
    tuple[bool | None, bool | None, int | None]
        ``(has_z, has_m, type_code)``.  All three are ``None`` when the
        payload is too short or the byte-order byte is not 0/1 — callers
    ...
    if bo == 1:
        ...
        return (None, None, None)
    code = int.from_bytes(wkb_data[1:5], endian)

    # PostGIS EWKB high-bit dimension flags
    if code & 0x80000000 or code & 0x40000000:
        return (bool(code & 0x80000000), bool(code & 0x40000000), code)
...
def _decode_gpkg_geom_header(blob):
    ...
    Implements the header layout defined in OGC 12-128r15 \u00a72.1.3 and is
    called by ``_r24()`` for every sampled geometry BLOB.
    ...
    Header layout::
        ...
        byte  2    : version
        byte  3    : flags
        bytes 4-7  : srs_id  (int32, byte order per flags bit 0)
        bytes 8-N  : optional envelope  (0 / 32 / 48 / 64 bytes)

    Flags byte (byte 3) bit layout::

        bit  0 (mask 0x01) : byte order of the header  (1 = little-endian)
        ...
        bit  4 (mask 0x10) : empty-geometry flag
        bit  5 (mask 0x20) : ExtendedGeoPackageBinary marker

    v1.60 fix (critical): this function previously reported
    ``has_z = flags & 0x02`` and ``has_m = flags & 0x04``.  There are no Z or
    M flags in the GeoPackage header \u2014 bits 1-3 are the *envelope indicator*,
    so those masks were reading envelope bits.  The practical effect was a
    false Req 24 "Z/M consistency" failure on ordinary files: any 2D geometry
    written with an XY envelope (envelope code 1, which GDAL and QGIS write
    for every non-point layer) has bit 1 set and was reported as
    "declared z=0 (prohibited) but header Z-flag=1", while a genuine 3D
    geometry stored without an envelope was reported as having no Z.  Z/M is
    now derived from the WKB geometry type code via ``_wkb_zm_flags()``.
    ...
    dict or None
        ...
        * ``magic_ok``       - True when bytes 0-1 are ``GP``
        * ``version``        - header version byte
        * ``flags``          - raw flags byte (int)
        * ``byte_order``     - 1 = little-endian header, 0 = big-endian
        * ``srs_id``         - signed int32 CRS identifier from the header
        * ``envelope_code``  - envelope indicator value (0-7)
        * ``envelope_valid`` - False when the indicator is a reserved value
        * ``is_empty``       - True when the empty-geometry flag is set
        * ``extended``       - True when the ExtendedGeoPackageBinary bit is set
        * ``header_size``    - total header length in bytes
        * ``has_z``          - True / False / None (None = could not decode)
        * ``has_m``          - True / False / None (None = could not decode)
        * ``wkb_type``       - raw WKB geometry type code, or None
    ...
    srs_id         = struct.unpack("<i" if byte_order else ">i", raw[4:8])[0]
    envelope_code  = (flags >> 1) & 0x07
    envelope_size  = _GPKG_ENVELOPE_SIZES.get(envelope_code)
    envelope_valid = envelope_size is not None
    if not envelope_valid:
        # Reserved indicator - the payload offset is unknowable.  Fall back to
        # the no-envelope offset and let the caller report it via
        # envelope_valid rather than silently decoding garbage as geometry.
        envelope_size = 0
    header_size = 8 + envelope_size
    wkb_data    = raw[header_size:]
    has_z, has_m, wkb_type = _wkb_zm_flags(wkb_data)
    return {
        "magic_ok":       raw[:2] == b"GP",
        "version":        raw[2],
        "flags":          flags,
...
def _parse_wkt_structural(wkt, srs_id, pyproj_crs_class):
    """Parse a WKT1 or WKT2 CRS string and cross-check it against EPSG.
    ...
    ----------
    wkt : str
        WKT1 or WKT2 CRS definition string from ``gpkg_spatial_ref_sys``.
    srs_id : int
    ...
    try:
        ...
        issues.append(
            f"pyproj.CRS.from_wkt() failed — WKT may be malformed: "
            f"{_wkt_parse_err}"
        )
...
def coverage_gaps(
    ...
    """Return the PASS* rows whose reason means the tool could not fully check.

    (v1.61) A PASS* row carries a `reason` code (see constants.PASSSTAR_*).
    Some reasons (SAMPLED, EVIDENCE_MISSING) mean the automated check did not
    have complete evidence — these are genuine coverage gaps and should block
    a clean "CONFORMANT" verdict. Others (ADVISORY) mean the check completed
    fully and is just leaving an informational note — those do NOT block.

    Returns a sorted list of (req_num, reason) tuples for the blocking rows.
    An empty list means every PASS* in `results` (if any) was advisory-only,
    i.e. the automated check achieved full coverage everywhere it passed.
    """
    out = []
    for k, v in results.items():
        ...
        if v.get("status") != "PASS*":
            continue
        reason = v.get("reason") or PASSSTAR_DEFAULT_REASON
        if reason in PASSSTAR_BLOCKING_REASONS:
            out.append((k, reason))
    return sorted(out)


def advisory_reqs(results: dict) -> list[int]:
    """Return the requirement numbers whose PASS* is advisory-only (v1.61).

    These are PASS* rows where the automated check is complete (no coverage
    gap) but is leaving an informational note or asking for a quick manual
    confirmation of something the tool cannot judge automatically. They do
    not affect the CONFORMANT verdict.
    """
    return sorted(
        k for k, v in results.items()
        if isinstance(k, int) and v.get("status") == "PASS*"
        and (v.get("reason") or PASSSTAR_DEFAULT_REASON) not in PASSSTAR_BLOCKING_REASONS
    )
...
def score_results(
    ...
    """Score a results dict into (counts, total, verdict, verdict_colour).

    (v1.61) The verdict ladder changed from a binary "zero PASS* required"
    rule (which made CONFORMANT structurally unreachable — every PASS* used
    to block it, even ones from fully-verified, informational-only checks)
    to a 3-tier ladder driven by reason codes:

      NON-CONFORMANT               — at least one requirement FAILed.
      CONFORMANT — REDUCED COVERAGE — no FAILs, but at least one requirement
                                       could not be fully verified (a
                                       coverage gap: PASS* tagged SAMPLED or
                                       EVIDENCE_MISSING).
      CONFORMANT (automated scope)  — no FAILs and no coverage gaps. Every
                                       PASS* present (if any) is advisory-only
                                       and does not affect the result.

    This is a like-for-like replacement for the old ("CONFORMANT" /
    "LIKELY CONFORMANT (partial checks)" / "NON-CONFORMANT") ladder; the
    difference is what makes a check count as "incomplete" — before, any
    PASS* did; now, only a PASS* that is actually missing evidence does.
    """
    counts = {"PASS": 0, "FAIL": 0, "ERROR": 0, "PASS*": 0, "SKIPPED": 0, "N/A": 0, "WARNING": 0}
    # v1.59 fix: select requirement rows by key TYPE instead of by a hardcoded
    # blocklist of dunder names.  The old blocklist had to be updated by hand
    # every time a new "__something__" side-channel key was attached to the
    # results dict (__internet__, __forensic__, __forensic_checks__,
    # __cascade_note__ so far).  Miss one and this loop does v["status"] on a
    # plain string, raising "TypeError: string indices must be integers" and
    # taking down the whole report.  Requirement keys are always ints.
    for k, v in results.items():
        if not isinstance(k, int):
            continue
        counts[v["status"]] = counts.get(v["status"], 0) + 1
    # v1.50 fix: derive total from counts, not len(results).
    # len(results) includes the extra string keys excluded from counts but
    # counted by len() — that gave 40 instead of 37.
    ...
    if error:
        ...
        verdict_color = "#8e0000"
    elif fail:
        verdict = "NON-CONFORMANT (automated checks)"
        verdict_color = "#e74c3c"
    elif gaps:
        verdict = "PRE-VALIDATION PASSED — REDUCED COVERAGE"
        verdict_color = "#f39c12"
    elif skipped:
        verdict = "PRE-VALIDATION PASSED — EXTERNAL CHECKS REQUIRED"
        verdict_color = "#2980b9"
    else:
        verdict = "CONFORMANT (automated scope)"
...
def pick_files_dialog() -> list[str]:
    ...
    try:
        ...
        # v1.60 fix: this hint named "DGIWG_Report_Generator.py", a file that
        # does not exist in the distribution — the same stale-filename problem
        # v1.59 fixed in the --help epilog.  Show the real entry points.
        print("  ⚠  tkinter is not available on this system.")
        print("     Install it or pass a file/folder path as a command-line argument:")
        print("     python -m dgiwg_validator your_file.gpkg")
        print("     python -m dgiwg_validator \"C:\\data\\gpkg_folder\"")
        print("     python DGIWG_Validator_v1_64.py your_file.gpkg")
        return []
