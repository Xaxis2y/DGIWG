...
"""Requirement check functions for DGIWG STD-DP-19-005 v1.1 (v1.64).
...
from .constants import (
    ...
    PASSSTAR_SAMPLED, PASSSTAR_EVIDENCE_MISSING, PASSSTAR_ADVISORY,
    PASSSTAR_DEFAULT_REASON,
)
from .net import (
    _net_get, _net_check_uri, _net_check_epsg, _net_check_scale_denoms,
    _net_check_dmf_xml, DMF_XSD_INLINE, _DMF_REQUIRED_ELEMENTS,
    # v1.59 cleanup: _http_get / _http_head were imported here but never called
...
def check_req(cursor, req_num):
    """Returns (status, detail_string, reason) for a single requirement.

    (v1.61) Individual _rN() check functions may return either a legacy
    2-tuple (status, detail) or a new 3-tuple (status, detail, reason). This
    normalizes both shapes into a 3-tuple: any PASS* without an explicit
    reason falls back to PASSSTAR_DEFAULT_REASON (ADVISORY) so it never
    silently becomes a coverage gap, and every non-PASS* status carries
    reason=None.
    """
    try:
        result = _CHECK_DISPATCH[req_num](cursor)
    except Exception as e:
        return ("ERROR", f"Validator exception during check: {type(e).__name__}: {e}", None)

    if len(result) == 3:
        status, detail, reason = result
    else:
        status, detail = result
        reason = None

    if status == "PASS*":
        reason = reason or PASSSTAR_DEFAULT_REASON
    else:
        reason = None
...
def _r3(cursor):
    ...
    v1.47 additions:
      ...
      (b) SQLite / GeoPackage base format checks (OGC core requirements
          inherited by DGIWG):
          • application_id/user_version — must identify GeoPackage 1.4.x as
            required by DGIWG STD-DP-19-005 v1.1.
    ...
    try:
        # OGC GeoPackage 1.4 uses the stable "GPKG" application identifier;
        # the encoded version is carried by PRAGMA user_version (10400 = 1.4.0).
        cursor.execute("PRAGMA application_id")
        _app_id = cursor.fetchone()[0]
        cursor.execute("PRAGMA user_version")
        _user_version = cursor.fetchone()[0]
        if _app_id != 0x47504B47:
            _legacy = {
                0x47503130: "GP10",
                0x47503131: "GP11",
                0x47503132: "GP12",
            ...
            fails.append(
                f"application_id=0x{_app_id:08X} ({_legacy}) is invalid for GeoPackage 1.4; "
                "expected 0x47504B47 ('GPKG')"
            )
        elif not (10400 <= _user_version < 10500):
            fails.append(
                f"user_version={_user_version} is not GeoPackage 1.4.x; "
                "DGIWG STD-DP-19-005 v1.1 requires 10400 through 10499"
            )
        else:
            passes.append(
                f"GeoPackage 1.4.x identifier confirmed: application_id=0x{_app_id:08X}, "
                f"user_version={_user_version} ✓"
            )
    except Exception as _ai_e:
        fails.append(f"application_id/user_version could not be verified: {_ai_e}")
...
def _r4(cursor):
    ...
    v1.59 change: this check used to return PASS* unconditionally, which made
    ...
    # v1.61: the automated check here (which optional extensions are
    # registered) is complete — this is an advisory note asking for a human
    # profile-permission judgement, not a coverage gap.
    return ("PASS*",
            "; ".join(notes)
...
def _r5(cursor):
    ...
    for ext, rules in EXTENSIONS_TABLE.items():
        for cat in cats:
            if rules.get(cat) == "N":
                matched = [e for e in present if e == ext]
                if matched:
                    fails.append(f"{ext} is NOT ALLOWED for {cat} but found: {matched}")
                else:
                    passes.append(f"{ext} correctly absent for {cat}")
    ...
    # v1.61: track how much of each tile table's data was actually scanned so
    # a WEBP-clean result can be tagged PASS (fully scanned) rather than
    # PASS*/SAMPLED (partially scanned) when the table has <= the sample size.
    # v1.61 fix: this LIMIT used to be hardcoded to 10, silently ignoring
    # --sample-size (the same class of bug fixed for Req 24/26 in v1.60/v1.61)
    # — a user could never close this coverage gap by re-running with a
    # larger sample. Now honours _config.SAMPLE_SIZE, defaulting to 10.
    _webp_sample_n = _config.SAMPLE_SIZE if _config.SAMPLE_SIZE else 10
    _webp_sampled = 0
    _webp_total = 0
    if "tiles" in cats:
        try:
            ...
            for tname in tile_tables:
                try:
                    cursor.execute(
                        ...
                        f'WHERE tile_data IS NOT NULL LIMIT ?',
                        (_webp_sample_n,)
                    )
                    ...
                    if blobs:
                        _webp_sampled += len(blobs)
                        try:
                            cursor.execute(
                                f'SELECT COUNT(*) FROM {_quote_ident(tname)} '
                                f'WHERE tile_data IS NOT NULL'
                            )
                            _webp_total += cursor.fetchone()[0]
                        except Exception:
                            _webp_total += len(blobs)
    ...
    _detail = "; ".join(passes) if passes else "No disallowed extensions or tile formats present"
    # v1.61: only tag SAMPLED (a coverage gap) when the tile scan genuinely
    # saw fewer BLOBs than exist — a table with <=10 tiles was scanned in
    # full and should return a bare PASS, not an unreachable-CONFORMANT PASS*.
    if _webp_sampled and _webp_sampled < _webp_total:
        return ("PASS*",
                _detail + f" — tile-format scan read {_webp_sampled} of "
                          f"{_webp_total} stored tile BLOB(s); a WEBP tile "
                          f"outside the sample would not have been seen",
                PASSSTAR_SAMPLED)
    if _webp_sampled:
        _detail += f" — tile-format scan read all {_webp_total} stored tile BLOB(s)"
    return ("PASS", _detail)
...
def _r7(cursor):
    ...
    _def063_expr = (
        "COALESCE(srs.definition_12_063,'')"
        if column_exists(cursor, "gpkg_spatial_ref_sys", "definition_12_063")
        else "''"
    )
    cursor.execute(f"""
        SELECT tms.srs_id, srs.srs_name, tms.table_name,
               COALESCE(srs.definition,''), {_def063_expr}
...
def _r8(cursor):
    ...
    return (
        "PASS*",
        detail + "\nStructural subset passed. Full DGIWG Req 8 evidence still requires "
        "the OGC 17-083r2 Two Dimensional Tile Matrix Set ATS A.1-A.16 and "
        "well-known scale-set verification.",
        PASSSTAR_EVIDENCE_MISSING,
    )

def _validate_vector_crs(cursor, z_clause, allowed_crs, dimension_label):
    """Validate vector CRS registration and every GeoPackageBinary header."""
    if not table_exists(cursor, "gpkg_geometry_columns"):
        return ("SKIPPED", "No vector geometry columns found")
    cursor.execute(f"""
        SELECT gc.table_name, gc.column_name, gc.srs_id, srs.srs_name
    ...
    if not rows:
        return ("SKIPPED", f"No {dimension_label} vector feature sets")

    cache = _load_epsg_json_cache()
    fails, passes = [], []
    for table, column, srs_id, srs_name in rows:
        label = f"{table}.{column}"
        if srs_id not in allowed_crs:
            fails.append(
                f"{label}: srs_id={srs_id} ({srs_name}) is not in the allowed "
                f"{dimension_label} vector CRS list"
            )
        elif not (srs_name or "").strip():
            fails.append(f"{label}: srs_id={srs_id} has a missing or empty srs_name")
        else:
            expected = cache.get(str(srs_id), {}).get("name")
            norm_actual = " ".join(str(srs_name).split()).casefold()
            norm_expected = " ".join(str(expected).split()).casefold() if expected else None
            if norm_expected and norm_actual != norm_expected:
                fails.append(
                    f"{label}: srs_name='{srs_name}' does not match the registered "
                    f"name '{expected}' for EPSG:{srs_id}"
                )

        qtable = _quote_ident(table)
        qcolumn = _quote_ident(column)
        try:
            cursor.execute(f"SELECT {qcolumn} FROM {qtable} WHERE {qcolumn} IS NOT NULL")
            inspected = 0
            while True:
                ...
                for (blob,) in batch:
                    inspected += 1
                    try:
                        header = _decode_gpkg_geom_header(blob)
                    except Exception as exc:
                        fails.append(f"{label} row {inspected}: geometry header decode error: {exc}")
                        continue
                    if header is None:
                        fails.append(f"{label} row {inspected}: geometry BLOB is shorter than 8 bytes")
                    elif not header["magic_ok"]:
                        fails.append(f"{label} row {inspected}: geometry BLOB does not begin with GP magic")
                    elif not header["is_empty"] and header["srs_id"] != srs_id:
                        fails.append(
                            f"{label} row {inspected}: GeoPackageBinary srs_id={header['srs_id']} "
                            f"does not match gpkg_geometry_columns.srs_id={srs_id}"
                        )
            passes.append(
                f"{label}: declared EPSG:{srs_id}; inspected all {inspected} non-NULL "
                "geometry header(s) ✓"
            )
        except sqlite3.Error as exc:
            fails.append(f"{label}: could not inspect geometry headers: {exc}")

    return ("FAIL" if fails else "PASS", "\n".join(fails + passes))


def _r9(cursor):
    """2D Vector CRS, registered name, and binary-header consistency."""
    return _validate_vector_crs(cursor, "gc.z = 0", ALLOWED_VECTOR_CRS_2D, "2D")


def _r10(cursor):
    """3D Vector CRS, registered name, and binary-header consistency."""
    return _validate_vector_crs(cursor, "gc.z != 0", ALLOWED_VECTOR_CRS_3D, "3D")
...
def _r12(cursor):
    ...
    # v1.61: every 3D gridded table found was checked against the allowlist
    # above — exhaustive, so a clean result is a genuine PASS.
...
def _r13(cursor):
    ...
    for _al_sid, _al_sname, _al_defn, _al_063 in rows:
        if _al_sid in _al_allowed:
            passes.append(
                f"srs_id={_al_sid} ({_al_sname}) is DGIWG Tables 1–12 approved ✓"
            )
        else:
            hard_fails.append(
    ...
    for srs_id, srs_name, defn, defn_063 in rows:
        ...
        # DGIWG permits either ISO 19162 WKT2 or the WKT1 representation in the
        # CRS extension column. Syntax alone is therefore not a failure.
        if any(kw in wkt_upper for kw in ("GEOGCS[", "PROJCS[", "COMPD_CS[", "VERT_CS[")):
            warnings.append("WKT1 syntax detected and accepted by the DGIWG profile")
        else:
            warnings.append("WKT2 syntax detected")
        ...
        if "AXIS" in wkt_upper:
            ...
            # v1.61 fix: matching "GEOGCRS[" anywhere in the WKT wrongly
            # classified every PROJCRS as geographic too, because WKT2 embeds
            # the projected CRS's base geographic CRS as "BASEGEOGCRS[...]" —
            # which CONTAINS "GEOGCRS[" as a substring starting at its 5th
            # character.  That made this axis-order check (which only makes
            # sense for a top-level geographic CRS, per DGIWG Tables 15/16)
            # run against projected CRSs like EPSG:3857 as well, where the
            # axes are Easting/Northing rather than Latitude/Longitude — an
            # unwinnable match that always produced a PASS*-forcing warning.
            # A WKT's outermost keyword identifies its actual CRS type, so
            # check only the start of the (whitespace-trimmed) string.
            _wkt_head = wkt_upper.lstrip()
            _is_geog = any(_wkt_head.startswith(k) for k in
                           ("GEOGCRS[", "GEOGCS[", "GEODCRS[", "GEODETICCRS["))
            if _is_geog:
                # Extract all AXIS[name, direction] pairs in document order.
                # v1.61 fix: the previous pattern required the direction word
                # to be immediately followed by the AXIS[...] block's closing
                # "]", which only matches WKT1-style entries like
                # AXIS["Lat",NORTH].  Real-world WKT2 (as emitted by PROJ/GDAL,
                # which this tool explicitly detects as a producer) instead
                # writes AXIS["geodetic latitude (Lat)",north,ORDER[1],
                # ANGLEUNIT[...]] — extra sub-elements between the direction
                # word and the closing bracket that the old pattern could
                # never match, so this check silently failed to parse axes
                # for almost every modern file. It now only requires the
                # direction word to immediately follow the axis name.
        ...
        if wkt_datum_name and pyproj_ok:
            try:
    ...
    if hard_fails:
        return ("FAIL", detail)
    if soft_warns:
        # v1.61: "warnings" mixes two very different things — a genuine
        # reduced-evidence note (pyproj unavailable, so the datum cross-check
        # fell back to an offline table or was skipped) and a pure
        # informational confirmation (axis order verified compliant, a datum
        # alias matched, a datum name matched exactly) that the full pyproj
        # cross-check produced along the way. When pyproj ran, every row's
        # hard rules AND the deep structural/datum check both completed —
        # any remaining "warning" text is informational, not a coverage gap.
        # Only when pyproj was unavailable is the evidence genuinely reduced.
        if pyproj_ok:
            return ("PASS*",
                    detail + "\n(PASS* = hard rules passed; full pyproj structural/datum "
                             "cross-check completed; remaining notes above are informational)",
                    PASSSTAR_ADVISORY)
...
def _r14(cursor):
    ...
    if not table_exists(cursor, "gpkg_geometry_columns"):
        if compound_srs:
            # v1.61: compound CRS is declared in the SRS table but there are no
            # vector geometry columns to check z-dimension applicability
            # against — the tool genuinely cannot verify this, so it's a
            ...
            return ("PASS*", f"Compound CRS present in SRS table: {'; '.join(compound_srs)}. "
                    ...
                    PASSSTAR_EVIDENCE_MISSING)
        # v1.61: nothing to check at all — no compound CRS declared and no
        # vector geometry columns present. This requirement doesn't apply to
        # this file's content, which is what SKIPPED means.
        return ("SKIPPED", "No vector geometry columns and no compound CRS in SRS — "
    ...
    rows = cursor.fetchall()
    if not rows:
        # v1.61: geometry columns table exists but is empty — nothing to check.
        return ("SKIPPED", "No feature geometry rows — compound CRS check not applicable")
    fails, passes = [], []
    ...
    # v1.61: every row was checked above — exhaustive over this file's
    # geometry columns, so a clean result is a genuine PASS. (The deeper WKT
    # structure check in Req 15 is a separate requirement, not missing
    # evidence for this one.)
    return ("PASS", "; ".join(passes) + " — full compound CRS WKT structure check in Req 15")


def _compound_wkt_components(wkt):
    """Return (syntax, horizontal, vertical, datum_present, issues)."""
    wkt_up = (wkt or "").upper()
    issues = []
    if "COMPOUNDCRS[" in wkt_up:
        syntax = "WKT2"
        horizontal = "GEOGCRS" if "GEOGCRS[" in wkt_up else (
            "PROJCRS" if "PROJCRS[" in wkt_up else None
        )
        vertical = "VERTCRS" if "VERTCRS[" in wkt_up else None
        datum_present = bool(re.search(r'VDATUM\["[^"]+"', wkt, re.IGNORECASE))
        if horizontal is None:
            issues.append("missing WKT2 horizontal GEOGCRS/PROJCRS component")
        if vertical is None:
            issues.append("missing WKT2 vertical VERTCRS component")
        if vertical and not datum_present:
            issues.append("missing non-empty VDATUM in VERTCRS")
    elif "COMPD_CS[" in wkt_up:
        syntax = "WKT1"
        horizontal = "GEOGCS" if "GEOGCS[" in wkt_up else (
            "PROJCS" if "PROJCS[" in wkt_up else None
        )
        vertical = "VERT_CS" if "VERT_CS[" in wkt_up else None
        datum_present = bool(re.search(r'VERT_DATUM\["[^"]+"', wkt, re.IGNORECASE))
        if horizontal is None:
            issues.append("missing WKT1 horizontal GEOGCS/PROJCS component")
        if vertical is None:
            issues.append("missing WKT1 vertical VERT_CS component")
        if vertical and not datum_present:
            issues.append("missing non-empty VERT_DATUM in VERT_CS")
    else:
        syntax, horizontal, vertical, datum_present = None, None, None, False
        issues.append("missing COMPOUNDCRS (WKT2) or COMPD_CS (WKT1) root")
    return syntax, horizontal, vertical, datum_present, issues


def _r15(cursor):
    """Validate WKT1 or WKT2 compound CRS used by vector feature tables."""
    if not table_exists(cursor, "gpkg_geometry_columns"):
    ...
    select_063 = ", definition_12_063" if has_063 else ""
    cursor.execute(
        f"SELECT srs_id, srs_name, definition{select_063} FROM gpkg_spatial_ref_sys "
        f"WHERE srs_id IN ({placeholders})", list(vector_srs_ids)
    )
    srs_rows = [tuple(r) + (() if has_063 else (None,)) for r in cursor.fetchall()]

    entries = []
    for srs_id, srs_name, definition, definition_12_063 in srs_rows:
        candidates = ((definition_12_063, "definition_12_063"), (definition, "definition"))
        selected = next(
            ((text, source) for text, source in candidates
             if text and any(k in text.upper() for k in ("COMPOUNDCRS[", "COMPD_CS["))),
            None,
        )
        if selected:
            entries.append((srs_id, srs_name, selected[0], selected[1]))
    if not entries:
        return ("SKIPPED", "No compound CRS found for vector feature tables — Req 15 not applicable")

    fails, passes = [], []
    for srs_id, srs_name, wkt, source in entries:
        syntax, horizontal, vertical, _datum, issues = _compound_wkt_components(wkt)
        label = f"srs_id={srs_id} ({srs_name})"
        if issues:
            fails.append(f"{label}: " + "; ".join(issues))
        else:
            passes.append(
                f"{label}: {syntax} compound structure accepted from {source}: "
...
def _r16(cursor):
    """Validate WKT1 or WKT2 gridded compound CRS in definition_12_063."""
    ...
    for tname, srs_id, srs_name, defn, defn_063 in rows:
        def_is_compound = bool(defn and any(k in defn.upper() for k in ("COMPOUNDCRS[", "COMPD_CS[")))
        ext_is_compound = bool(defn_063 and any(k in defn_063.upper() for k in ("COMPOUNDCRS[", "COMPD_CS[")))
        if def_is_compound or ext_is_compound:
            compound_entries.append((tname, srs_id, srs_name, defn_063, ext_is_compound))
        else:
    ...
    hard_fails, passes = [], []
    for tname, srs_id, srs_name, wkt, ext_is_compound in compound_entries:
        label  = f"gridded table '{tname}' srs_id={srs_id} ({srs_name})"
        if not has_063:
            issues = ["definition_12_063 column is absent"]
        elif not ext_is_compound:
            issues = ["definition_12_063 does not contain the required compound WKT"]
        else:
            _syntax, _horizontal, _vertical, _datum, issues = _compound_wkt_components(wkt)
        if issues:
            hard_fails.append(f"{label}: " + "; ".join(issues))
        else:
            syntax, horizontal, vertical, _datum, _issues = _compound_wkt_components(wkt)
            passes.append(
                f"{label}: {syntax} compound structure accepted from definition_12_063: "
                f"{horizontal} + {vertical} ✓"
            )
...
def _r18(cursor):
    """GeoPackage Metadata DMF — deterministic field-level pre-validation.
    ...
    The bundled XSD is intentionally only a screening schema, not the complete
    DGIWG DMF schema set. A clean result is therefore always PASS* and requires
    external DMF schema/ATS evidence before a conformity claim.
    """
    ...
    # v1.59 fix: the XSD step used to disappear without trace when lxml was
    ...
    for row_id, md_scope, uri, mime, xml_text in dmf_rows:
        ...
        if _lxml_ok:
            ...
            if not _xsd_ok:
                row_warns.append(
                    f"{label}: bundled screening XSD did not accept the document ({_xsd_msg}); "
                    "this is not treated as a definitive DMF failure"
                )
            else:
                ok.append(_xsd_msg)
        else:
        ...
        # ISO 19115-3 uses a different namespace topology from legacy GMD. The
        # legacy XPath checks below cannot safely judge that model, so retain
        # the well-formedness result and route the record to the official DMF ATS.
        if "standards.iso.org/iso/19115" in str(root.tag):
            passes.append(
                f"{label}: ISO 19115-3 metadata is well-formed; field-level legacy GMD "
                "checks are not applicable"
            )
            row_warns.append(
                f"{label}: full DGIWG DMF schema and ATS validation remains required"
            )
            continue
    ...
    if passes:
        return (
            "PASS*",
            detail + "\nOfficial DGIWG DMF schema and ATS validation is still required.",
            PASSSTAR_EVIDENCE_MISSING,
        )
    return ("PASS*", detail, PASSSTAR_EVIDENCE_MISSING)
...
def _r20(cursor):
    ...
    # v1.61: the automated part of this check (required columns, row count,
    # mime_type) is complete for what it covers — the geopackage-scope
    # reference-row note above is informational, not a coverage gap, so this
    # is an advisory PASS* rather than a blocking one.
    detail += ". Automated column/row/mime_type checks complete; the geopackage-scope reference note above is informational only."
    return ("PASS*", detail, PASSSTAR_ADVISORY)
...
def _r24(cursor):
    ...
    # v1.60 fix: accept every gridded data_type spelling already tolerated by
    # Req 11/12/25/26.  Listing only "2d-gridded-coverage" here meant a gridded
    # file written with the "2d_gridded_coverage" / "gpkg_2d_gridded_coverage"
    # spelling skipped ATS 5.4/5.5 entirely, so a genuinely missing
    # gpkg_tile_matrix_set or gpkg_tile_matrix was reported as "not required".
    has_tile_tables = any(
        dt.strip().lower() in ("tiles", "2d-gridded-coverage",
    ...
    if not shapely_ok:
        ...
        for geom_tname, geom_col, declared_z, declared_m, declared_geom_type in geom_tables:
            ...
            zm_mismatches   = []  # v1.47: Z/M vs declared column mismatches
            zm_undecodable  = 0   # v1.60: blobs whose WKB type code could not be read
            ...
            for idx, blob in enumerate(blobs, 1):
                try:
                    ...
                    if not hdr["magic_ok"]:
                        errors.append(
                            f"geom {idx}: not a GeoPackage geometry BLOB — "
                            f"bytes 0-1 are not the 'GP' magic marker"
                        )
                        continue
                    if not hdr["envelope_valid"]:
                        errors.append(
                            f"geom {idx}: flags byte declares reserved envelope "
                            f"indicator {hdr['envelope_code']} (valid values are 0-4) "
                            f"— WKB payload offset cannot be determined"
                        )
                        continue

                    # ── Z/M consistency check (v1.47, corrected in v1.60) ────
                    # Z/M presence is carried by the WKB geometry TYPE CODE
                    # (ISO thousands offset, or EWKB high bits), not by the
                    # GeoPackage flags byte — bits 1-3 of that byte are the
                    # envelope indicator.  Reading them as Z/M flags made every
                    # 2D layer written with an XY envelope (the GDAL/QGIS
                    # default) report a spurious "declared z=0 but Z-flag=1".
                    # gpkg_geometry_columns.z: 0=no-Z, 1=mandatory-Z, 2=optional-Z
                    # gpkg_geometry_columns.m: 0=no-M, 1=mandatory-M, 2=optional-M
                    # z/m = 2 means either state is permitted — no check.
                    blob_has_z = hdr["has_z"]
                    blob_has_m = hdr["has_m"]
                    if blob_has_z is None or blob_has_m is None:
                        zm_undecodable += 1
                    else:
                        if declared_z == 1 and not blob_has_z:
                            zm_mismatches.append(
                                f"geom {idx}: declared z=1 (mandatory) but WKB type "
                                f"{hdr['wkb_type']} carries no Z"
                            )
                        elif declared_z == 0 and blob_has_z:
                            zm_mismatches.append(
                                f"geom {idx}: declared z=0 (prohibited) but WKB type "
                                f"{hdr['wkb_type']} carries Z"
                            )
                        if declared_m == 1 and not blob_has_m:
                            zm_mismatches.append(
                                f"geom {idx}: declared m=1 (mandatory) but WKB type "
                                f"{hdr['wkb_type']} carries no M"
                            )
                        elif declared_m == 0 and blob_has_m:
                            zm_mismatches.append(
                                f"geom {idx}: declared m=0 (prohibited) but WKB type "
                                f"{hdr['wkb_type']} carries M"
                            )
            ...
            if zm_undecodable:
                geom_skips.append(
                    f"'{geom_tname}'.{geom_col}: Z/M dimension not verifiable for "
                    f"{zm_undecodable} sampled geometry BLOB(s) — WKB geometry type "
                    f"code could not be read"
                )
            if type_mismatches:
                geom_fails.append(
...
def _r26(cursor):
    ...
    if not pillow_ok:
        ...
        # v1.60 fix: --sample-size was ignored on this path, contradicting its
        # own --help text ("Number of geometry/tile BLOBs to sample per table
        # for Req 24 and Req 26").  An explicit --sample-size is now honoured;
        # the lighter default of 3 applies only when the flag is absent.
        SAMPLE_N_FB = _config.SAMPLE_SIZE if _config.SAMPLE_SIZE else 3
        fb_summaries = []
        fb_all_pass  = True
        fb_any_data  = False
        fb_any_parsed = False
        ...
        if not fb_any_parsed:
            ...
            return ("PASS*",
                    ...
                    PASSSTAR_EVIDENCE_MISSING)
        if fb_all_pass:
            # v1.61: still PASS* (not PASS) even though every sampled tile
            # matched — header parsing (no Pillow) is inherently lighter
            # evidence than a full pixel decode, regardless of how much of
            # the data was sampled, so this always counts as a coverage gap.
            return ("PASS*", detail_fb, PASSSTAR_EVIDENCE_MISSING)
    ...
    # v1.61: track true coverage (stored BLOBs seen vs. total stored BLOBs) so
    # a table with <= SAMPLE_N tiles per zoom level — fully decoded — can
    # return a bare PASS instead of an unreachable-CONFORMANT PASS*.
    _cov_total = 0
    _cov_seen = 0
    ...
    for tname, zoom_levels in sorted(by_table.items()):
        ...
        for zoom, declared_w, declared_h in zoom_levels:
            ...
            try:
                cursor.execute(f"""
                    SELECT COUNT(*) FROM {_quote_ident(tname)}
                    WHERE zoom_level = ? AND tile_data IS NOT NULL
                """, (zoom,))
                _cov_total += cursor.fetchone()[0]
            except Exception:
                _cov_total += len(blobs)
            mismatches = []
            decode_errors = []
            # v1.60 fix: the zoom-level success line below used to interpolate
            # actual_w/actual_h, loop variables left over from the last decoded
            # BLOB.  Tracked explicitly so the message can never quote a size
            ...
            for idx, blob in enumerate(blobs, 1):
                try:
                    ...
                    verified_size = (actual_w, actual_h)
            ...
            if mismatches:
                ...
                _vs = (f"{verified_size[0]}×{verified_size[1]}"
                       if verified_size else f"{declared_w}×{declared_h}")
                zoom_results.append(
                    f"  zoom {zoom}: ✅ {sampled} tile(s) sampled — "
                    f"all {_vs} match declared {declared_w}×{declared_h}"
                )
    ...
    if all_pass:
        # v1.61: a table with <= SAMPLE_N stored tiles per zoom level was
        # decoded in full (_cov_seen >= _cov_total) — a genuine PASS. A
        # larger table where only the sample was decoded keeps PASS*/SAMPLED,
        # since a wrong-sized tile outside the sample would not have been seen.
        if _cov_seen >= _cov_total:
            return ("PASS",
                    detail + f"\n\nCoverage: all {_cov_total} stored tile BLOB(s) decoded.")
        return ("PASS*",
                detail + f"\n\nCoverage: {_cov_seen} of {_cov_total} stored tile "
                         f"BLOB(s) decoded ({SAMPLE_N} per zoom level). A "
                         f"wrong-sized tile outside the sample would not have "
                         f"been seen.",
                PASSSTAR_SAMPLED)
    else:
        return ("FAIL", detail)
...
def _r28(cursor):
    ...
    if fails:
        return ("FAIL", "; ".join(fails))
    # v1.61: every tile table in `by_table` was checked above for monotonic
    # pixel-size reduction — exhaustive, so a clean result is a genuine PASS.
...
def _r29(cursor):
    ...
    if fails:
        return ("FAIL", "; ".join(fails))
    # v1.61: every table in `tables` was checked above for tile-dimension
    # consistency across zoom levels — exhaustive, so a clean result is a
    # genuine PASS.
    return ("PASS", "; ".join(passes))
...
def _r31(cursor):
    ...
    layer_fails = []
    tile_layer_tables = set()  # v1.61: defensive init in case the query below raises
    try:
        cursor.execute(
            "SELECT table_name FROM gpkg_contents WHERE data_type='tiles'"
        )
    ...
    if not rows and not layer_fails:
        if not tile_layer_tables:
            # v1.61: no tile layers exist in this GeoPackage at all — the
            # requirement genuinely does not apply, not just "unverified".
            return ("SKIPPED",
                    "No tile layers (data_type='tiles') in this GeoPackage — "
                    "Req 31 not applicable")
        # v1.61: tile layers exist and are all linked (checked above via
        # layer_fails, which is empty), but no metadata document actually
        # declares md_scope='tile'/'model' content to inspect — this is an
        # advisory note, not a coverage gap, since the linkage check is complete.
        return ("PASS*",
                "gpkg_metadata_reference exists ✓ | "
                f"all {len(tile_layer_tables)} tile layer(s) are linked by a "
                "reference_scope='table' row, but no metadata document declares "
                "md_scope='tile' or 'model' — confirm by hand that the linked "
                "documents do describe the tile layers",
                PASSSTAR_ADVISORY)
...
def _r32(cursor):
    ...
    # ── v1.47: per-layer completeness check ──────────────────────────────────
    layer_fails = []
    feature_layer_tables = set()  # v1.61: defensive init in case the query below raises
    try:
        cursor.execute(
    ...
    if not rows and not layer_fails:
        if not feature_layer_tables:
            # v1.61: no feature layers exist in this GeoPackage at all — the
            # requirement genuinely does not apply, not just "unverified".
            return ("SKIPPED",
                    "No feature layers (data_type='features') in this GeoPackage — "
                    "Req 32 not applicable")
        # v1.61: feature layers exist and are all linked (checked above via
        # layer_fails, which is empty), but no metadata document actually
        # declares md_scope='featureType'/'feature' content to inspect — this
        # is an advisory note, not a coverage gap, since the linkage check is
        # complete.
        return ("PASS*",
                "gpkg_metadata_reference exists ✓ | "
                f"all {len(feature_layer_tables)} feature layer(s) are linked by a "
                "reference_scope='table' row, but no metadata document declares "
                "md_scope='featureType' or 'feature' — confirm by hand that the "
                "linked documents do describe the feature layers",
                PASSSTAR_ADVISORY)
...
def _r33(cursor):
    ...
    # v1.60 fix: gpkg_extensions is optional in the OGC core spec.  This check
    # queried it unguarded, so a gridded GeoPackage without that table raised
    # "no such table" and check_req() turned it into an opaque generic-exception
    # FAIL.  This is the same Bug B class fixed for Req 3/4/5 in v1.58; Req 33
    # was missed.  Absence simply means the extension is not registered.
    if table_exists(cursor, "gpkg_extensions"):
        cursor.execute("SELECT extension_name FROM gpkg_extensions")
        present = {r[0] for r in cursor.fetchall()}
    else:
    ...
    elif not table_exists(cursor, "gpkg_extensions"):
        fails.append("gpkg_extensions table is MISSING — the mandatory "
                     "gpkg_2d_gridded_coverage extension cannot be registered")
    else:
    ...
    if fails:
        return ("FAIL", "; ".join(fails))
    # v1.61: extension registration, ancillary table existence, and every
    # required column were all checked above — exhaustive, so a clean result
    # is a genuine PASS.
    return ("PASS", "; ".join(passes))
...
def _manual_checks(cursor):
    ...
    # ── v1.59 fix: normalise every requirement entry to a 4-tuple ─────────────
...
def run_all_checks(conn: sqlite3.Connection) -> dict[int, dict[str, object]]:
    ...
    for req_num in REQUIREMENTS:
        status, detail, reason = check_req(c, req_num)
        results[req_num] = {"status": status, "detail": detail, "reason": reason, "manual": None}
