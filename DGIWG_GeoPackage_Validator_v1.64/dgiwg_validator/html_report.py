...
"""Per-file HTML report rendering for the DGIWG validator (v1.60)."""
import os
import sys
import html
import json
...
from .constants import (
    ...
    OGC_PIXEL_Z0, OGC_PIXEL_TOL,
    PASSSTAR_REASON_META, PASSSTAR_DEFAULT_REASON,
)
from .forensics import _render_forensic_section
from .utils import score_results, coverage_gaps, advisory_reqs
...
def render_html(
    ...
    # v1.59 fix: filter requirement rows by key TYPE (int) rather than by a
    # hardcoded blocklist of "__dunder__" side-channel keys — see the same fix
    # in utils.score_results().  A key added later and forgotten here would
    # otherwise crash rendering with "string indices must be integers".
    manual_confirmed = sum(
        1 for k, v in results.items()
        if isinstance(k, int) and v["status"] == "PASS*" and v.get("manual")
        and v["manual"][0] == v["manual"][1] > 0
    )
    manual_partial = sum(
        1 for k, v in results.items()
        if isinstance(k, int) and v["status"] == "PASS*" and v.get("manual")
        and 0 < v["manual"][0] < v["manual"][1]
    )
    ...
    for req_num, (name, uri, compliance) in REQUIREMENTS.items():
        ...
        if status in ("ERROR", "SKIPPED"):
            ...
            conf = "High"
        conf_reason = "Confidence is derived only from this validation run and its available evidence."
        external = {
            1: "Confirm the current DGIWG GeoPackage profile and conformance class externally.",
            2: "Run the official DGIWG/OGC schema and ATS evidence checks externally.",
            6: "Confirm the intended product type and profile applicability with the data authority.",
            8: "Run OGC 17-083r2 Tile Matrix Set ATS A.1-A.16 and well-known scale-set checks.",
            18: "Validate metadata with the official DGIWG DMF schema set and ATS.",
        }
        auditor_chks = [external.get(req_num, "Review the current requirement detail and any Manual Confirmation panel.")]
        ...
        # ── v1.61: PASS* reason chip ─────────────────────────────────────────
        # Shows whether this PASS* is a coverage gap (blocks CONFORMANT) or an
        # advisory note (does not), using the same PASSSTAR_REASON_META the
        # scorer reads. Every PASS* row has a reason by the time it reaches
        # here — check_req() defaults an unclassified one to ADVISORY.
        reason_chip = ""
        if status == "PASS*":
            _reason = r.get("reason") or PASSSTAR_DEFAULT_REASON
            _rmeta  = PASSSTAR_REASON_META.get(_reason)
            if _rmeta:
                _rcolor = _rmeta["color"]
                _rlabel = _rmeta["short"]
                _rdesc  = html.escape(_rmeta["desc"])
                _rremedy = html.escape(_rmeta["remedy"])
                _blocks_note = (
                    "blocks CONFORMANT (automated scope)" if _rmeta["blocks"]
                    else "does not block CONFORMANT"
                )
                reason_chip = (
                    f'<div style="margin-top:4px;">'
                    f'<span style="background:{_rcolor};color:white;padding:2px 6px;'
                    f'border-radius:3px;font-size:10px;font-weight:bold;" '
                    f'title="{_rdesc} ({_blocks_note}). Remedy: {_rremedy}">'
                    f'{_rlabel}</span></div>'
                )
        ...
        <tr style="background:{bg};">
            ...
            <td style="padding:8px;text-align:center;">{status_badge}{reason_chip}</td>
    ...
    # ── v1.59: conformance-scope banner ──────────────────────────────────────
    # The verdict line at the top of this report says CONFORMANT / LIKELY
    # CONFORMANT / NON-CONFORMANT, and readers reasonably take that as the whole
    # answer.  It is not: Req 1, 2 and 6 are never executed by this tool (OGC
    # CITE TeamEngine and the applicable product profile are required), so a
    # clean verdict here is a statement about the 34 requirements this tool can
    # test — not a certificate of full DGIWG conformance.  Until v1.58 that
    # caveat lived only in the page footer, below everything else.  It now sits
    # directly under the verdict where the verdict is actually read.
    _skipped_scope = [
        r for r in (1, 2, 6)
        if results.get(r, {}).get("status") == "SKIPPED"
    ]
    if _skipped_scope:
        _scope_list = ", ".join(f"Req {r}" for r in _skipped_scope)
        _scope_banner = (
            '<div style="background:#eaf2f8;border:1px solid #aed6f1;border-left:5px solid #2980b9;'
            'border-radius:6px;padding:12px 18px;margin:16px 0;font-size:13px;color:#21618c;">'
            '<strong>ℹ️ Scope of this verdict</strong><br>'
            f'This verdict covers the {37 - len(_skipped_scope)} requirements this tool can test '
            f'automatically. {_scope_list} were not executed: Req 1 and Req 2 (OGC base conformance) '
            'require the OGC CITE TeamEngine test suites, and Req 6 (conditional extensions) depends '
            'on the applicable DGIWG product profile. '
            'Full conformance to STD-DP-19-005 v1.1 additionally requires those checks to be '
    ...
    # ── v1.61: coverage-gap banner ────────────────────────────────────────────
    # Replaces the old "any PASS* blocks CONFORMANT" behaviour with an explicit
    # explanation of exactly which requirements are keeping this file from
    # "CONFORMANT (automated scope)" and why — plus a note on the advisory
    # PASS* rows that are NOT holding the verdict back, so the two kinds of
    # PASS* are never conflated in the reader's eyes.
    _gaps = coverage_gaps(results)
    _advisories = advisory_reqs(results)
    if _gaps:
        _gap_items = "".join(
            f'<li style="margin:3px 0;"><strong>Req {n}:</strong> '
            f'{html.escape(PASSSTAR_REASON_META.get(reason, {}).get("label", reason))} — '
            f'{html.escape(PASSSTAR_REASON_META.get(reason, {}).get("remedy", ""))}</li>'
            for n, reason in _gaps
        )
        _coverage_banner = (
            '<div style="background:#fef5e7;border:2px solid #f39c12;border-radius:6px;'
            'padding:14px 20px;margin:18px 0;font-size:13px;color:#7d5a00;">'
            f'<strong>⚠️ Coverage Gaps — {len(_gaps)} requirement(s) not fully verified</strong><br>'
            'These requirements automated-passed on the data examined, but the tool could '
            'not gather complete evidence (a sample, or a missing library/row) — that is why '
            'the verdict above is "CONFORMANT — REDUCED COVERAGE" rather than '
            '"CONFORMANT (automated scope)":'
            f'<ul style="margin:8px 0 0;padding-left:20px;">{_gap_items}</ul>'
            '</div>'
        )
    else:
        _coverage_banner = ""
    if _advisories:
        _adv_list = ", ".join(f"Req {n}" for n in _advisories)
        _advisory_banner = (
            '<div style="background:#eaf2f8;border:1px solid #aed6f1;border-left:5px solid #2980b9;'
            'border-radius:6px;padding:12px 18px;margin:16px 0;font-size:13px;color:#21618c;">'
            f'<strong>ℹ️ Advisory notes — {len(_advisories)} requirement(s)</strong><br>'
            f'{_adv_list} passed with an informational note attached (see the reason chip in '
            'each row). The automated check for these is complete and did not affect the verdict.'
            '</div>'
        )
    else:
        _advisory_banner = ""
...
<div class="content">
  ...
  <div class="legend">
    ...
    <span class="legend-item"><span class="legend-dot" style="background:#f39c12;"></span> PASS* — Conditional pass (see reason chip: Sampled / Evidence missing / Advisory)</span>
    ...
    <span class="legend-item"><span style="background:#f39c12;color:white;padding:2px 6px;border-radius:3px;font-size:10px;font-weight:bold;">Sampled</span>/<span style="background:#f39c12;color:white;padding:2px 6px;border-radius:3px;font-size:10px;font-weight:bold;">Evidence missing</span> = coverage gap, blocks "CONFORMANT (automated scope)"; <span style="background:#3498db;color:white;padding:2px 6px;border-radius:3px;font-size:10px;font-weight:bold;">Advisory</span> = informational only, does not block</span>
  </div>
...
<div class="footer">
  ...
  PASS* rows tagged Sampled/Evidence missing are coverage gaps (see banner above); PASS* rows tagged Advisory are fully-checked informational notes.
  <div style="margin-top:8px;font-size:11px;color:#7f8c8d !important;">SPDX-License-Identifier: GPL-2.0-or-later &nbsp;|&nbsp; Copyright (c) 2026 Eui Soo SON</div>
</div>
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(html_str)
    # v1.59 fix: honour --quiet.  This line printed unconditionally, so
    # --quiet still emitted one "HTML report written" line per file and the
    # promised one-line-per-file summary output was not achievable in batch
    # runs piped into a log.
