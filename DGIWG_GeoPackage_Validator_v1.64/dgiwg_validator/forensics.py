...
"""Source-software forensics and deep-dive checks (v1.60).
...
def _fcheck_req13(conn, software):
    ...
    if not has_063:
        expl += ("Also: the WKT2-specific column 'definition_12_063' is absent from this "
                 "file. DGIWG requires the CRS extension column where applicable; permitted WKT1 or WKT2 may be used. "
        ...
        try:
            ...
            if full_meta and full_meta.strip().startswith("<"):
                ...
                if software == "ArcGIS":
                    ...
                    # v1.60 fix: the previous pattern list was
                    #   [r"c:\\", r"c:/", ".gdb\\", "geodatabase\\"]
                    # — raw strings, so those entries contained TWO literal
                    # backslashes and could never match a real Windows path
                    # (c:\gis\..., ...\data.gdb\...), which is written with one.
                    # This check therefore never fired.  Single backslash now,
                    # plus a drive-letter pattern that is not hardcoded to C:.
                    _INTERNAL_PATH_PATTERNS = [
                        "\\", "c:/", ".gdb", "geodatabase", "file:///",
                    ]
                    if any(p in lower_meta for p in _INTERNAL_PATH_PATTERNS):
