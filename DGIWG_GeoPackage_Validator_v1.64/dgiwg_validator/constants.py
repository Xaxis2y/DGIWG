...
"""Lookup tables and constants for the DGIWG validator (v1.60).
...
REQUIREMENTS = {
    ...
    # v1.58 fix: Req 1/2 names and identifiers aligned with STD-DP-19-005 v1.1
    # Table 6 (previously used pre-1.1 wording '/req/base/application' etc.).
    1:  ("GeoPackage Base definition",        "/req/geopackage/base",                   "M"),
    2:  ("GeoPackage Options definition",     "/req/geopackage/options",                "M"),
    ...
    # v1.58 fix: identifier aligned with STD-DP-19-005 v1.1 (/req/crs/crs-compound-wkt)
    15: ("Compound CRS WKT",                  "/req/crs/crs-compound-wkt",              "M"),
...
# (v1.57 fix #12: duplicate 'import math as _math' removed — already imported at top)
_WGS84_A = 6378137.0
OGC_PIXEL_Z0 = {
...
# ──────────────────────────────────────────────────────────────────────────────
# PASS* REASON CODES (v1.61)
#
# Before v1.61, "PASS*" meant one single thing: "conditionally passed, read the
# detail text to find out why." That collapsed two very different situations
# into one status:
#   (a) the tool genuinely could not fully verify the requirement (it sampled
#       data, evidence was missing, or the check is not fully automatable) —
#       this SHOULD block a clean "CONFORMANT" verdict, because the tool is
#       telling you it does not have complete evidence either way; and
#   (b) the tool fully verified the requirement and is just flagging something
#       advisory or informational alongside a genuine pass — this should NOT
#       block "CONFORMANT", because the automated check is complete.
#
# Every PASS* now carries one of the reason codes below so score_results() can
# tell these apart. This generalizes the `unverifiable` 4th tuple element that
# _manual_checks() already used (since v1.59) for the same purpose.
# ──────────────────────────────────────────────────────────────────────────────
PASSSTAR_SAMPLED          = "SAMPLED"           # only part of the data was examined (e.g. LIMIT-N scan); a violation outside the sample would not have been seen
PASSSTAR_EVIDENCE_MISSING = "EVIDENCE_MISSING"  # the automated check could not gather enough evidence to fully verify (fallback path, missing optional library, absent row/column, etc.)
PASSSTAR_ADVISORY         = "ADVISORY"          # the automated check is complete; this is an informational/manual-confirmation note, not a coverage gap
PASSSTAR_DEFAULT_REASON   = PASSSTAR_ADVISORY   # fallback for any legacy/unclassified PASS* site so nothing silently becomes a coverage gap by accident

PASSSTAR_REASON_META = {
    PASSSTAR_SAMPLED: {
        "label":  "Sampled evidence",
        "short":  "Sampled",
        "blocks": True,
        "color":  "#f39c12",
        "desc":   "Only a sample of the relevant data was examined (e.g. a "
                  "LIMIT-N scan). A violation outside the sampled rows/tiles "
                  "would not have been detected.",
        "remedy": "Re-run with a larger --sample-size, or inspect the "
                   "un-sampled rows/tiles by hand to close the gap.",
    },
    PASSSTAR_EVIDENCE_MISSING: {
        ...
        "blocks": True,
        "color":  "#f39c12",
        "desc":   "The automated check could not gather enough evidence to "
                  "fully verify this requirement (a fallback code path ran, "
                  "an optional library was unavailable, or an expected "
                  "row/column was absent).",
        "remedy": "Install the optional library / supply the missing data, "
                   "then re-run, or verify this requirement by hand.",
    },
    PASSSTAR_ADVISORY: {
        "label":  "Advisory note",
        "short":  "Advisory",
        "blocks": False,
        "color":  "#3498db",
        "desc":   "The automated check is complete and found no violation. "
                  "This note is informational or asks for a quick manual "
                  "confirmation of something the tool cannot judge "
                  "automatically (e.g. semantic correctness).",
        "remedy": "Optional: review the note for a quick manual "
                   "confirmation. Does not affect the CONFORMANT verdict.",
    },
}

PASSSTAR_BLOCKING_REASONS = frozenset(
    r for r, m in PASSSTAR_REASON_META.items() if m["blocks"]
)
...
MANUAL_GUIDANCE = {
    ...
    # v1.58 fix: UTM zones removed — STD-DP-19-005 v1.1 Table 13 permits only
    # EPSG:4979 and EPSG:9518 for 3D vector features (matches ALLOWED_VECTOR_CRS_3D).
    10: ("All 3D vector layers must use EPSG:4979 or EPSG:9518 (WGS 84 + EGM2008 height) per DGIWG Table 13.",
...
REQ_ROLLUP_META = {
    ...
    1: {
        "name": "GeoPackage Base definition",
    ...
    2: {
        "name": "GeoPackage Options definition",
        "auditor_checks": ["Cannot be checked automatically — requires OGC CITE TeamEngine external test harness."],
    ...
    3: {
        ...
        "confidence": "High",
        "confidence_reason": "Legacy metadata only; current reports derive confidence from the active validation run.",
    },
    4: {
        ...
        "manual_notes": "Legacy metadata only; current reports do not apply historical corpus evidence.",
        "confidence": "Medium",
    ...
    6: {
        ...
        "confidence": "Low",
        "confidence_reason": "Requires product profile context that may not be available in the file.",
    },
    7: {
    ...
    9: {
        ...
        "manual_notes": "Review current-file vector evidence and any manual confirmation panel.",
        "confidence": "High",
    ...
    10: {
        ...
        "manual_notes": "Review current-file 3D vector applicability and evidence.",
        "confidence": "Low",
        "confidence_reason": "Confidence must be derived from current-run applicability and evidence.",
    },
    11: {
        "name": "Gridded 2D CRS",
        "auditor_checks": [
    ...
    14: {
        ...
        "manual_notes": "Review current-run PASS* reasons and manual confirmations.",
        "confidence": "Medium",
    ...
    16: {
        ...
        "auditor_checks": [
            ...
            "FAIL if definition_12_063 is absent when required; accept permitted WKT1 or WKT2 compound syntax",
        ],
...
LIKELY_CAUSE: dict = {
    ...
    10: {
        # v1.58 fix: UTM wording removed — Table 13 allows only 4979 / 9518 for 3D vector.
        "QGIS/GDAL": "3D vector feature table uses an unsupported CRS. Must be EPSG:4979 or EPSG:9518.",
        "ArcGIS":    "3D vector feature table uses an unsupported CRS. ArcGIS may assign a custom srs_id for reprojected 3D data. Must be EPSG:4979 or EPSG:9518.",
        "UNKNOWN":   "3D vector feature table does not use a DGIWG-approved 3D CRS (EPSG:4979 or EPSG:9518).",
    },
    11: {
    ...
    16: {
        ...
        "ArcGIS":    "definition_12_063 is absent or does not contain a valid WKT1/WKT2 compound CRS for gridded data.",
