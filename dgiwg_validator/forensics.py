...
def _fcheck_req7(conn, software):
    ...
    for r in rows:
        ...
        if ok:
            flag = f"✅ EPSG:{srs_id} — on DGIWG allowed list"
            ...
            flag = (f"⚠️ srs_id={srs_id} — not on DGIWG Table 9 approved list "
                    f"{sorted(ALLOWED_RASTER_CRS)}. Verify and reproject if needed.")
            bad_ids.append(srs_id)
    ...
    if not bad_ids:
        return _forensic_result(_F_PASS,
            ...
            data_rows,
            f"Every tile layer uses a CRS from the DGIWG Table 9 approved list "
            f"({sorted(ALLOWED_RASTER_CRS)}). Req 7 is satisfied.", sql)

    expl = (f"CRS problem found on {len(bad_ids)} layer(s): srs_id values {bad_ids}. "
            "DGIWG Req 7 says raster data must use only the approved CRS codes from "
            f"Table 9: {sorted(ALLOWED_RASTER_CRS)}. ")
...
def run_forensic_checks(conn, software):
    ...
    return {
        "REQ5":  _fcheck_req5(conn, software),
        "REQ7":  _fcheck_req7(conn, software),
        "REQ9":  _fcheck_req9(conn, software),
        "REQ13": _fcheck_req13(conn, software),
        "REQ18": _fcheck_req18(conn, software),
        "REQ19": _fcheck_req19(conn, software),
        "REQ22": _fcheck_req22(conn, software),
        "REQ23": _fcheck_req23(conn, software),
        "REQ24": _fcheck_req24(conn, software),
    }
...
# ──────────────────────────────────────────────────────────────────────────────
# MANUAL (DEEPER) CHECKS
# For every requirement that returns PASS*, we run a second, deeper DB query
# that confirms what was only partially checked.  Returns (confirmed, total, note).
# confirmed = number of items that passed the deeper check
# total     = number of items that were checked
