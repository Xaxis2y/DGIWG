...
from .constants import (
    ...
    ALLOWED_RASTER_CRS, ALLOWED_GRIDDED_2D_CRS, ALLOWED_GRIDDED_3D_CRS,
...
from .net import (
    ...
    _http_get, _http_head, _load_epsg_json_cache,
)
from .utils import (
...
def check_req(cursor, req_num):
    ...
    try:
        ...
        return ("FAIL", f"Exception during check: {e}")
...
def _r3(cursor):
    ...
    try:
        ...
        # 0x47504B47 = "GPKG" — used by older GDAL/QGIS (pre-3.x) and some ArcGIS
        # versions before OGC finalised the version encoding scheme.  Recognised
        # but not conformant with OGC GeoPackage 1.x spec.
        _KNOWN_NONSTANDARD = {
            0x47504B47: "pre-standard GDAL/QGIS 'GPKG' marker (non-conformant)",
        }
        if _app_id in _GP_VERS:
        ...
        elif _app_id in _KNOWN_NONSTANDARD:
            fails.append(
                f"application_id=0x{_app_id:08X} — {_KNOWN_NONSTANDARD[_app_id]}; "
                f"DGIWG requires 0x47503132 (GeoPackage 1.2/1.3)"
            )
        elif _app_id == 0:
            ...
            fails.append(
                ...
                f"expected 0x47503132 (GeoPackage 1.2/1.3)"
            )
    ...
    try:
        ...
        _now = _dtlc.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S")
...
def _r4(cursor):
    ...
    return ("PASS*", "; ".join(notes) if notes else "No optional extensions registered")
...
def _r7(cursor):
    ...
    for srs_id, srs_name, tname in tms_rows:
        ...
        if srs_id in ALLOWED_RASTER_CRS:
            ...
            crs_fails.append(f"{label}: srs_id={srs_id} ({srs_name}) NOT in DGIWG raster CRS list")
    ...
    for srs_id, srs_name, defn, defn_063 in rows:
        ...
        if wkt_datum_name and pyproj_ok:
            ...
            if _expected:
                ...
                _net_datum = None
                try:
                    from builtins import _DGIWG_OFFLINE  # type: ignore
                    _offline = bool(_DGIWG_OFFLINE)
                except (ImportError, AttributeError):
                    _offline = False
                if not _offline:
                    try:
                        _ok, _body = _net_get(_EPSG_API.format(code=srs_id), timeout=NET_TIMEOUT)
...
def _r18(cursor):
    ...
    try:
        ...
        pass  # lxml not installed — XSD validation silently skipped
    ...
    all_lines = hard_fails + passes + row_warns
    detail    = "\n".join(all_lines)
...
def _r24(cursor):
    ...
    """
    import builtins as _b24
    GEOM_SAMPLE_N = getattr(_b24, "_DGIWG_SAMPLE_SIZE", 25)
    import re as _re24
                    ...
                    if _decl_bbox is not None:
                        ...
                        if (_ax_min < _dx_min - _TOL or _ay_min < _dy_min - _TOL or
                            ...
                            geom_passes.append(
                                ...
                                f"(full index aggregate, all {_feat_count if '_feat_count' in dir() else '?'} features)"
                            )
                    else:
                        geom_passes.append(
...
def _r26(cursor):
    ...
    """
    import builtins as _b26
    SAMPLE_N = getattr(_b26, "_DGIWG_SAMPLE_SIZE", 5)  # BLOBs to sample per zoom level per tile table
...
def _r1(_cursor):
    """OGC Base Application — requires OGC CITE TeamEngine, not automatable."""
    return ("SKIPPED",
            "Req 1 (OGC Base Application): Requires OGC CITE TeamEngine — not automatable")

def _r2(_cursor):
    """OGC Base GeoPackage — requires OGC CITE TeamEngine, not automatable."""
    return ("SKIPPED",
            "Req 2 (OGC Base GeoPackage): Requires OGC CITE TeamEngine — not automatable")
...
def _manual_checks(cursor):
    ...
    try:
        ...
        if not multi_sets:
            ...
            for tname in multi_sets:
                ...
                cursor.execute(
                    f'SELECT zoom_level, COUNT(*) FROM "{tname}" GROUP BY zoom_level',)
    ...
    try:
        ...
        if not all_tm:
            ...
            for tname, zooms in single_tables.items():
                ...
                cursor.execute(
                    f'SELECT COUNT(*) FROM "{tname}" WHERE zoom_level=?', (zoom,))
    ...
    import builtins as _b_uri_offline
    if getattr(_b_uri_offline, "_DGIWG_OFFLINE", False):
        mc["uri"] = (None, None,
...
def run_all_checks(conn: sqlite3.Connection) -> dict[int, dict[str, object]]:
    ...
    for req_num, mc in manual_data.items():
        if isinstance(req_num, str):
            ...
            results[req_num]["manual"] = mc   # (confirmed, total, note)
