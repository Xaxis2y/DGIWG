...
REM
REM  Output:
REM    dist\DGIWG_Validator_v1_61.exe
REM ============================================================
...
if not exist "DGIWG_Validator_v1_61.py" (
    echo   ERROR: DGIWG_Validator_v1_61.py not found in this folder.
    echo   ERROR: DGIWG_Validator_v1_61.py not found. >> "%LOG%"
    pause
    exit /b 1
)
...
if exist "dist\DGIWG_Validator_v1_61.exe" del /f /q "dist\DGIWG_Validator_v1_61.exe" 2>>"%LOG%"
echo   OK.
...
if not exist "dist\DGIWG_Validator_v1_61.exe" (
    echo   ERROR: build finished but dist\DGIWG_Validator_v1_61.exe was not
    echo   created - see %LOG%.
    echo   ERROR: exe not found after build. >> "%LOG%"
    pause
    exit /b 1
)
echo   OK - dist\DGIWG_Validator_v1_61.exe created.
echo   OK - dist\DGIWG_Validator_v1_61.exe created. >> "%LOG%"

echo.
echo [6/6] Smoke-testing the exe ^(--version^) ...
echo [6/6] Smoke-testing the exe ^(--version^) ... >> "%LOG%"
"dist\DGIWG_Validator_v1_61.exe" --version >> "%LOG%" 2>&1
if errorlevel 1 (
...
for %%F in ("dist\DGIWG_Validator_v1_61.exe") do set EXESIZE=%%~zF
echo   exe size: %EXESIZE% bytes >> "%LOG%"

echo.
echo ============================================================
echo  Build complete.
echo  EXE      : %cd%\dist\DGIWG_Validator_v1_61.exe
echo  Log file : %LOG%
echo.
echo  Try it now:
echo    dist\DGIWG_Validator_v1_61.exe --help
echo    dist\DGIWG_Validator_v1_61.exe path\to\file.gpkg
echo    dist\DGIWG_Validator_v1_61.exe            ^(opens file picker^)
echo ============================================================
echo.
pause
endlocal
