...
"""Requirement check functions for DGIWG STD-DP-19-005 v1.1 (v1.59).
...
def _r7(cursor):
    ...
    cursor.execute("""
        SELECT tms.srs_id, srs.srs_name, tms.table_name,
               COALESCE(srs.definition,''), COALESCE(srs.definition_12_063,'')
...
def _r18(cursor):
    ...
    if hard_fails:
        return ("FAIL", detail)
    if passes:
        # v1.59: without lxml the deepest structural stage did not run, so a
